...
if [[ $NofPROJECTs == 0 ]]; then 
	echo -e ${COL1}${BEG}${COL2}
	echo -e ${COL1_WARN}"◼︎ CHECKPOINT ERROR - no project selected in "${MASTERPROJECTS}${COL2}" to be run"
	echo -e ${COL1}${END}${COL2}
	touch ./error
	exit
fi
...
cat ${MASTERPROJECTS} | awk -F"\t" '$1=="yes"' | while read -r line || [[ -n "$line" ]]; do
	...
	check_space_in_master "${MASTERPROJECTS}" "READ_COUNT_MODE" "${READ_COUNT_MODE}" "${PROJECT_ID}"
	...
	READ_COUNT_FILTER_VALUES=("default" "F" "DF")
	...
	check_value_in_master "${MASTERPROJECTS}" "READ_COUNT_MODE" "${READ_COUNT_FILTER}" "${READ_COUNT_FILTER_VALUES[*]}" "${PROJECT_ID}" 
