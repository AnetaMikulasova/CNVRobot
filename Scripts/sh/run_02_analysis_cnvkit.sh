...
cat ${MASTERPROJECTS} | awk -F"\t" '$1=="yes"' | while read -r line || [[ -n "$line" ]]; do
	...
	GERMLINE_MODE="${SETTING_MODE:1:1}"
	# AUTOSOME_MODE="${SETTING_MODE:2:1}"
	#for CNVkit, it will be always != "A" as the doubling is a part of the tool; although it is different than GATK as chrY is always haploid
	AUTOSOME_MODE="CNVKIT"
	QUICK_MODE="${SETTING_MODE:3:1}"
	QUICK_MODE_PLOT="${SETTING_MODE:4:1}"
	CNVKIT_MODE="${SETTING_MODE:5:1}"
	ASCAT_MODE="${SETTING_MODE:6:1}"

	[[ ${READ_COUNT_FILTER} == "-" ]] && READ_COUNT_FILTER="F"
	[[ -z ${GERMLINE_MODE} ]] && GERMLINE_MODE="-"
	[[ -z ${AUTOSOME_MODE} ]] && AUTOSOME_MODE="-"
	[[ -z ${QUICK_MODE} ]] && QUICK_MODE="-"
	[[ -z ${QUICK_MODE_PLOT} ]] && QUICK_MODE_PLOT="-"
	[[ -z ${CNVKIT_MODE} ]] && CNVKIT_MODE="-"
	[[ -z ${ASCAT_MODE} ]] && ASCAT_MODE="-"

	if [[ ${GERMLINE_MODE} != "G" ]] && [[ $AUTOSOME_MODE != "A" ]]; then 
		MODE_PATTERN=$(echo $READ_COUNT_FILTER)
	fi
	if [[ ${GERMLINE_MODE} == "G" ]] && [[ $AUTOSOME_MODE == "A" ]]; then 
		MODE_PATTERN=$(echo $READ_COUNT_FILTER"-"$GERMLINE_MODE"-"$AUTOSOME_MODE)
	fi
	if [[ ${GERMLINE_MODE} == "G" ]] && [[ $AUTOSOME_MODE != "A" ]]; then 
		MODE_PATTERN=$(echo $READ_COUNT_FILTER"-"$GERMLINE_MODE)
	fi
	if [[ ${GERMLINE_MODE} != "G" ]] && [[ $AUTOSOME_MODE == "A" ]]; then 
		MODE_PATTERN=$(echo $READ_COUNT_FILTER"-"$AUTOSOME_MODE)
	fi
	###====================================================================================
	...
	CTRL_COUNT_CNN_DIR=${SUPPORT_FILES}"Controls/counts/cnn/"
...
#----CNVKIT----
				#colect CNVkit coverage
				#============================================================================
				OUTPUT=${RESULTS_DATA_PATTERN}"_targetcoverage.cnn"
				OUTPUT_ANTI=${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn"
				#CHECK - READ COUNTS
				if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
					#CHECK - AS CTRL EXISTS
					if ls ${CTRL_COUNT_COUNTS_DIR}"/cnn/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_targetcoverage.cnn" 1> /dev/null 2>&1; then
						echo -e ${COL1}${BEG}${COL2}
						echo -e ${COL1}"◼︎ $(date)"${COL2}
						echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit collecting coverage"${COL2} 
						echo -e ${COL1}"◼︎ collected as control, copying data..."${COL2} 							
						echo -e ${COL1}${END}${COL2}
						cp ${CTRL_COUNT_COUNTS_DIR}"/cnn/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_targetcoverage.cnn" ${OUTPUT}
						cp ${CTRL_COUNT_COUNTS_DIR}"/cnn/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_antitargetcoverage.cnn" ${OUTPUT_ANTI}
					else
						echo -e ${COL1}${BEG}${COL2}
						echo -e ${COL1}"◼︎ $(date)"${COL2}
						echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit collecting coverage"${COL2} 
				...
				# if [[ ${AUTOSOME_MODE} != "A" && ${CASE_SEX} == "M" ]]; then
				# 	OUTPUT=${RESULTS_DATA_PATTERN}"_targetcoverage_doublegon.cnn"
				# 	#CHECK - READ COUNTS
				# 	if ! ls ${OUTPUT} 1> /dev/null 2>&1; then

				# 		#CHECK - AS CTRL EXISTS
				# 		if ls ${CTRL_COUNT_COUNTS_DIR}"cnn/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_targetcoverage_doublegon.cnn" 1> /dev/null 2>&1; then
				# 			echo -e ${COL1}${BEG}${COL2}
				# 			echo -e ${COL1}"◼︎ $(date)"${COL2}
				# 			echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit - collecting read count - hdf5 double gonosomes"${COL2} 
				# 			echo -e ${COL1}"◼︎ collected as control, copying data..."${COL2} 							
				# 			echo -e ${COL1}${END}${COL2}
				# 			cp ${CTRL_COUNT_COUNTS_DIR}"cnn/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_targetcoverage_doublegon.cnn" ${OUTPUT}
				# 		else
				# 			echo -e ${COL1}${BEG}${COL2}
				# 			echo -e ${COL1}"◼︎ $(date)"${COL2}
				# 			echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit - doubling gonosomal coverage for this male sample"${COL2}
				# 			echo -e ${COL1}${END}${COL2}
				# 			Rscript --vanilla ${R}"cnvkit_gonosome_double.R" \
				# 				${RESULTS_DATA_PATTERN}"_targetcoverage.cnn" \
				# 				${OUTPUT}
				...
				# 					echo -e "   Main ID: "${MAIN_ID}
				# 					echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
				# 					echo -e "   Stage: CNVkit - doubling gonosomal counts for male sample"
				# 					echo -e "   File not generated: "${OUTPUT}
...
#----CNVKIT----
							#prepare pre-file with corrected sex for controls with unk sex
							rm -f ${MASTERCONTROLS}"_temp"*
							cat ${MASTERCONTROLS} | awk -F"\t" -v PROJECT_ID="${PROJECT_ID}" -v CAPTURE_ID="${CAPTURE_ID}" -v GENOME_VERSION="${GENOME_VERSION}" '{if ($1=="yes" && $2==PROJECT_ID && $3==CAPTURE_ID && $4==GENOME_VERSION)print}' > ${MASTERCONTROLS}"_temp1"
							touch ${MASTERCONTROLS}"_temp2"
							while IFS= read -r line; do
								defined_sex=$(echo "$line" | awk -F"\t" '{print $7}')
								file=$(echo "$line" | awk -F'\t' -v CTRL_COUNT_CNN_DIR="${CTRL_COUNT_CNN_DIR}" '{print CTRL_COUNT_CNN_DIR $5 "_" $6 "_" $7 "_targetcoverage.cnn"}')
								if [[ $defined_sex == "unk" && ! -e "$file" ]]; then
									base_filename=${file%_unk_targetcoverage.cnn}
									[[ -e ${base_filename}"_M_targetcoverage.cnn" ]] && NEWSEX="M"
									[[ -e ${base_filename}"_F_targetcoverage.cnn" ]] && NEWSEX="F"
									if [[ $NEWSEX != $defined_sex ]]; then
							            line=$(echo "$line" | awk -v newsex="$NEWSEX" -F"\t" 'BEGIN{OFS="\t"} {$7=newsex; print}')
							        fi
								fi
								echo "$line" >> ${MASTERCONTROLS}"_temp2"
							done < ${MASTERCONTROLS}"_temp1"
...
#----CNVKIT----
							...
							if [[ ${GERMLINE_MODE} != "G" ]] || ( [[ ${GERMLINE_MODE} == "G" ]] && [[ ${CASE_ID} == ${SAMPLE_2_ID} ]] ) && [[ ${PON_SEX} == "mixed" ]]; then
								
								hdflist=$(awk -F'\t' -v CTRL_COUNT_HDF_DIR="${CTRL_COUNT_HDF_DIR}" '{
								    if ($7 == "M") {
								        print CTRL_COUNT_HDF_DIR $5 "_" $6 "_" $7 "_counts_doublegon.hdf5";
								    } else {
								        print CTRL_COUNT_HDF_DIR $5 "_" $6 "_" $7 "_counts.hdf5";
								    }
								}' ${MASTERCONTROLS}"_temp3" | sed 's/^/-I /')
								hdflist_doublegon=""
...
#----CNVKIT----
							reference_cnvkit_target=`awk -F'\t' -v CTRL_COUNT_CNN_DIR="${CTRL_COUNT_CNN_DIR}" '{print CTRL_COUNT_CNN_DIR $5 "_" $6 "_" $7 "_targetcoverage.cnn"}' ${MASTERCONTROLS}"_temp3" | sed 's/^/ /'`
							reference_cnvkit_antitarget=`awk -F'\t' -v CTRL_COUNT_CNN_DIR="${CTRL_COUNT_CNN_DIR}" '{print CTRL_COUNT_CNN_DIR $5 "_" $6 "_" $7 "_antitargetcoverage.cnn"}' ${MASTERCONTROLS}"_temp3" | sed 's/^/ /'`
							# reference_cnvkit_target_doublegon=`awk -F'\t' -v CTRL_COUNT_CNN_DIR="${CTRL_COUNT_CNN_DIR}" '{print CTRL_COUNT_CNN_DIR $5 "_" $6 "_" $7 ""_targetcoverage_doublegon.cnn""}' ${MASTERCONTROLS}"_temp3" | sed 's/^/ /'`
#----CNVKIT----

							NRealx=`cat ${MASTERCONTROLS}"_temp3" | wc -l`
						...
						#============================================================================
#----CNVKIT----
						OUTPUT_DENOIS=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}".cnr"

						#CHECK - DENOIS
						if ! ls ${OUTPUT_DENOIS} 1> /dev/null 2>&1; then
							
							if [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
								OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline-"${CASE_SEX}".cnn"
							else
								OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"-"${CASE_SEX}".cnn"
							fi
							...
							if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ "${MAIN_ID}": CNVkit - preparing PoN-"${PON_SEX_ID}"-"${CASE_SEX}${COL2}
								echo -e ${COL1}${END}${COL2}

								echo ${reference_cnvkit_target} > ${OUTPUT}"_list.txt"

								source ${CONDA}
								conda activate ${CNVKIT_ENV}
								
								# [[ ${CASE_SEX} == "M" ]] && case_sex="male"
								# [[ ${CASE_SEX} != "M" ]] && case_sex="female"

								#CHECK - CTRL YES 
								if [[ ${NReal} != 0 ]]; then
									# cnvkit.py reference ${reference_cnvkit_target} ${reference_cnvkit_antitarget} -y -f ${REF} -o ${OUTPUT}
									#now running this on CTRL1 and CTRL11
									#cnvkit.py reference ${reference_cnvkit_target} ${reference_cnvkit_antitarget} -f ${REF} -o ${OUTPUT}
									#cnvkit.py reference ${reference_cnvkit_target} ${reference_cnvkit_antitarget} -y -f ${REF} -o ${OUTPUT}
									...
									if [[ ${CASE_SEX} == "M" ]]; then 
										cnvkit.py reference -y ${reference_cnvkit_target} ${reference_cnvkit_antitarget} -f ${REF} -o ${OUTPUT}
									fi
								...
								#CHECK - CTRL YES 
								if [[ ${NReal} == 0 ]]; then
									cnvkit.py reference -o ${OUTPUT} -f ${REF} -t ${INTERVALS_PROCESSED}"_target.bed" -a ${INTERVALS_PROCESSED}"_antitarget.bed"
								#CHECK - CTRL YES 
								fi
								...
								if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
										...
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
										echo -e "   Stage: CNVkit - panel of normals"
										echo -e "   File not generated: "${OUTPUT}
										echo -e ""${COL2}
										touch ./error
										exit
							...
							# #CHECK - CTRL YES 
							# if [[ ${NReal} != 0 ]]; then
							# 	#prepare secondary PON for situations with gonosomal double
							# 	if [[ ${AUTOSOME_MODE} != "A" ]]; then
							# 		if [[ ${PON_SEX_ID} == "M" ]] || [[ ${GERMLINE_MODE} == "G" && ${CASE_SEX} == "M" ]]; then
									
							# 			OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.cnn"
							# 			#CHECK - PON
							# 			if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
							# 				echo -e ${COL1}${BEG}${COL2}
							# 				echo -e ${COL1}"◼︎ $(date)"${COL2}
							# 				echo -e ${COL1}"◼︎ "${MAIN_ID}": CNVkit - preparing PoN-"${PON_SEX_ID}" - with gonosomal double"${COL2}
							# 				echo -e ${COL1}${END}${COL2}
							# 				# echo ${hdflist_doublegon} > ${OUTPUT}"_list.txt"
											
							# 				source ${CONDA}
							# 				conda activate ${CNVKIT_ENV}
							# 				cnvkit.py reference -y ${reference_cnvkit_target_doublegon} ${reference_cnvkit_antitarget} -f ${REF} -o ${OUTPUT}
							# 				conda deactivate
							# 				fi
							...
							# 						echo -e "   Main ID: "${MAIN_ID}
							# 						echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
							# 						echo -e "   Stage: CNVkit - panel of normals - with gonosomal double"
							# 						echo -e "   File not generated: "${OUTPUT}
							# 						echo -e ""${COL2}
							# 						touch ./error
							# 						exit
...
#----CNVKIT----
						...
						#============================================================================
#----CNVKIT----		
						OUTPUT=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}".cnr"
						
						if [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
							PANEL=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline-"${CASE_SEX}".cnn"
						else
							PANEL=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"-"${CASE_SEX}".cnn"
						fi

						# CHECK - PROCESSED DENOIS 
						if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
							#denoising using CNVkit, with or without reference (decided at the PoN creation level)
							echo -e ${COL1}${BEG}${COL2}
							echo -e ${COL1}"◼︎ $(date)"${COL2}
							echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit denoising using reference PoN-"${PON_SEX_ID}${COL2} 
							echo -e ${COL1}${END}${COL2}
							source ${CONDA}
							conda activate ${CNVKIT_ENV}
							
							cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${PANEL} -o ${OUTPUT}

							# #default CNVkit setting, or when no controls
							# if [[ ${AUTOSOME_MODE} == "A" ]] || [[ ${NReal} == 0 ]]; then
							# 	cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".cnn" -o ${OUTPUT}
							# fi

							# #prepare secondary PON for situations with gonosomal double
							# if [[ ${AUTOSOME_MODE} != "A" && ${NReal} != 0 ]] ; then

							# 	if [[ ${CASE_SEX} == "M" && ${PON_SEX_ID} == "M" ]]; then
							# 		cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage_doublegon.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.cnn" -o ${OUTPUT}
							# 	fi
								
							# 	if [[ ${CASE_SEX} == "M" && ${PON_SEX_ID} == "F" ]]; then
							# 		cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage_doublegon.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".cnn" -o ${OUTPUT}
							# 	fi
								
							# 	if [[ ${CASE_SEX} == "F" && ${PON_SEX_ID} == "M" ]]; then
							# 		cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.cnn" -o ${OUTPUT}
							# 	fi
								
							# 	if [[ ${CASE_SEX} == "unk" && ${PON_SEX_ID} == "M" ]]; then
							# 		cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.cnn" -o ${OUTPUT}
							# 	fi

							# 	if [[ ${GERMLINE_MODE} == "G" && ${CASE_SEX} == "M" ]]; then
							# 		cnvkit.py fix ${RESULTS_DATA_PATTERN}"_targetcoverage_doublegon.cnn" ${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn" ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.cnn" -o ${OUTPUT}
							# 	fi
							...
							if [[ ${CHECKPOINTS} == "yes" ]]; then 
								if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
									...
									echo -e "   Main ID: "${MAIN_ID}
									echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
									echo -e "   Stage: CNVkit sample denoising"
									echo -e "   File not generated: "${OUTPUT}
									echo -e ""${COL2}
									touch ./error
									exit
							...
							#delete PON cnn files to save some space
							rm -f ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"*".cnn"


							#transform data to simple format
							Rscript --vanilla ${R}"cnvkit_transform_denois.R" ${OUTPUT}
							...
							if [[ ${CHECKPOINTS} == "yes" ]]; then 
								if ls "status_ok" 1> /dev/null 2>&1; then
									...
									echo -e "   Main ID: "${MAIN_ID}
									echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
									echo -e "   Stage: CNVkit - transforming denoising data"
									echo -e "   File not generated: "${OUTPUT}
									echo -e "   R script: cnvkit_transform_denois.R"
									echo -e ""${COL2}
									touch ./error
									exit
								fi
...
#----CNVKIT----
						...
						#============================================================================
						#CHECK - SEGMENTATION
						OUTPUT=${RESULTS_DATA_PATTERN}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX_ID}"_"${SEGMENTATIONCONDITIONS}"_segmentation_CNVkit.tsv"

						if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
							if ! ls ${RESULTS_DATA_PATTERN}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX_ID}"_"${SEGMENTATIONCONDITIONS}"_CNVkit.tsv" 1> /dev/null 2>&1; then 
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit - segmentation and quality control, using PoN-"${PON_SEX_ID}${COL2} 
								echo -e ${COL1}${END}${COL2}
								mkdir -p ${QC_DIR}
								...
								if [[ ${SEGMENTATION_ID} == "smart-"* ]]; then	
									SMART_SEGMENTATION=`Rscript --vanilla ${R}"segmentation_conditions_smart.R" \
									${SEGMENTATION_ID} \
									${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}".cnr" \
									${INTERVALS_ANNOTATED} \
									${SEQ_TYPE}`
								...
								Rscript --vanilla ${R}"segmentation_and_qc.R" \
									...
									${PROJECT_ID} \
									${PROJECT_TYPE} \
									"cnvkit_I_own_you_this-raw_data_for_qc" \
								...
								if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ls "status_ok" 1> /dev/null 2>&1; then
										...
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
										echo -e "   Stage: CNVkit - data segmentation"
										echo -e "   File not generated: "${OUTPUT}
						...
						if [[ ${QUICK_MODE} != "Q" ]]; then
							OUTPUT=${RESULTS_DATA_PATTERN}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX_ID}"_"${SEGMENTATIONCONDITIONS}"_CNVkit.tsv"
							#CHECK - ANNOTATION DTB
							if ! ls ${OUTPUT} 1> /dev/null 2>&1; then 
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": CNVkit - incorporation of gene annotation, DGV and CTRLs to segmentation, using PoN-"${PON_SEX_ID}${COL2} 
								echo -e ${COL1}${END}${COL2}
								...
								Rscript --vanilla ${R}"annotation.R" \
									...
									${MAPPABILITY} \
									${ENCODE_BLACK_LIST} \
									${RESULTS_DATA_PATTERN}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX_ID}"_"${SEGMENTATIONCONDITIONS}"_segmentation_CNVkit.tsv" \
									"cnvkit_I_own_you_this-controls_incorporation" \
									${N} \
									${CTRL_PON_SEX_SELECT} \
									${SEGM_MINGAIN} \
									${SEGM_MINGAIN_SUB} \
								...
								if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ls "status_ok" 1> /dev/null 2>&1; then
										...
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
										echo -e "   Stage: CNVkit - annotation"
										echo -e "   File not generated: "${OUTPUT}
			...
			if [[ ${PON_SEX} != "NA" ]]; then
				...
				if [[ ${QUICK_MODE} != "Q" ]]; then
					...
					if [[ ${PROJECT_TYPE} == "germline" || ${PROJECT_TYPE} == "tumor" ]]; then
						OUTPUT=${RESULTS_DATA_DIR}${SAMPLE_1_TYPE}_${SAMPLE_1_ID}_${SAMPLE_1_SEX}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX}"_"${SEGMENTATIONCONDITIONS}"_REPORT_CNVkit.tsv"
						#CHECK - REPORT
						if ! ls ${OUTPUT} 1> /dev/null 2>&1; then 
							echo -e ${COL1}${BEG}${COL2}
							echo -e ${COL1}"◼︎ $(date)"${COL2}
							...
							Rscript --vanilla ${R}"create_report.R" \
								...
								${SAMPLE_1_SEX} \
								${PON_SEX_ID_SAMPLE1} \
								${RESULTS_DATA_DIR}${SAMPLE_1_TYPE}"_"${SAMPLE_1_ID}"_"${SAMPLE_1_SEX}"_SEGMENTS_C-"${N}"_PoN-"${PON_SEX_ID_SAMPLE1}"_"${SEGMENTATIONCONDITIONS}"_CNVkit.tsv" \
								${RESULTS_DATA_DIR}${SAMPLE_1_TYPE}"_"${SAMPLE_1_ID}"_"${SAMPLE_1_SEX}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID_SAMPLE1}".cnr" \
								${SAMPLE_2_TYPE}"_"${SAMPLE_2_ID} \
								${SAMPLE_2_SEX} \
								${PON_SEX_ID_SAMPLE2} \
								${RESULTS_DATA_DIR}${SAMPLE_2_TYPE}"_"${SAMPLE_2_ID}"_"${SAMPLE_2_SEX}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID_SAMPLE2}".cnr" \
								${SAMPLE_3_TYPE}"_"${SAMPLE_3_ID} \
								${SAMPLE_3_SEX} \
								${PON_SEX_ID_SAMPLE3} \
								${RESULTS_DATA_DIR}${SAMPLE_3_TYPE}"_"${SAMPLE_3_ID}"_"${SAMPLE_3_SEX}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID_SAMPLE3}".cnr" \
								${SEGM_MINGAIN} \
								${SEGM_MINGAIN_SUB} \
								${SEGM_MINLOSS} \
								${SEGM_MINLOSS_SUB} \
							...
							if [[ ${CHECKPOINTS} == "yes" ]]; then 
								if ls "status_ok" 1> /dev/null 2>&1; then
									...
									echo -e "   Stage: CNVkit - creating report"
									echo -e "   File not generated: "${OUTPUT}
				...
				RESULTS_PLOT_DIR=${RESULTS_DIR_GO}"plot_C-"${N}"_PoN-"${PON_SEX}"_"${SEGMENTATIONCONDITIONS}"/cnvkit_plot/"
				RESULTS_IGV_BED_DIR=${RESULTS_DIR_GO}"plot_C-"${N}"_PoN-"${PON_SEX}"_"${SEGMENTATIONCONDITIONS}"/cnvkit_IGV/"
				...
				if ! ls ${RESULTS_PLOT_DIR}${MAIN_ID}"_genome_profile_"${SAMPLE_1_TYPE}"_"${SAMPLE_1_ID}"_"${SAMPLE_1_SEX}".png" 1> /dev/null 2>&1; then
					...
					echo -e ${COL1}${BEG}${COL2}
					echo -e ${COL1}"◼︎ $(date)"${COL2}
					echo -e ${COL1}"◼︎ $MAIN_ID - $SAMPLE_1_TYPE $SAMPLE_1_ID: CNVkit - Creating plots - using PoN-"${PON_SEX}${COL2} 
					echo -e ${COL1}${END}${COL2}
