...
cat ${MASTERPROJECTS} | awk -F"\t" '$1=="yes"' | while read -r line || [[ -n "$line" ]]; do
		...
		if [[ ${CTRL} == "yes" ]]; then 
			cat ${MASTERCONTROLS} | awk -F"\t" -v PROJECT_ID="${PROJECT_ID}" -v CAPTURE_ID="${CAPTURE_ID}" -v GENOME_VERSION="${GENOME_VERSION}" '{if ($1=="no" && $2==PROJECT_ID && $3==CAPTURE_ID && $4==GENOME_VERSION)print}' | while read -r line || [[ -n "$line" ]]; do
				...
				if [[ ${CTRL_SEX} == "unk" ]]; then 
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_M_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="M"; fi
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_F_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="F"; fi
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_unk_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="unk"; fi
				fi
				RESULTS_DATA_PATTERN=${MAIN_ID}"_"${CTRL_ID}"_"${CTRL_SEX}

				rm -f ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5"
				rm -f ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5"
				# rm -f ${CTRL_COUNT_TSV_DIR}${RESULTS_DATA_PATTERN}"_counts.tsv"
				rm -f ${CTRL_MAF_DIR}${RESULTS_DATA_PATTERN}"_allelicCounts"${GNOMAD_COUNT_PATTERN}".rds"
				rm -f ${CTRL_SEX_DIR}"results"*

				rm -f ${CTRL_COUNT_CNN_DIR}${RESULTS_DATA_PATTERN}"_targetcoverage.cnn"
				# rm -f ${CTRL_COUNT_CNN_DIR}${RESULTS_DATA_PATTERN}"_targetcoverage_doublegon.cnn"
				rm -f ${CTRL_COUNT_CNN_DIR}${RESULTS_DATA_PATTERN}"_antitargetcoverage.cnn"
		...
		if [[ ${CTRL} == "yes" ]]; then
			...
			cat ${MASTERCONTROLS} | awk -F"\t" -v PROJECT_ID="${PROJECT_ID}" -v CAPTURE_ID="${CAPTURE_ID}" -v GENOME_VERSION="${GENOME_VERSION}" '{if ($1=="yes" && $2==PROJECT_ID && $3==CAPTURE_ID && $4==GENOME_VERSION)print}' | while read -r line || [[ -n "$line" ]]; do
				...
				for FILE_TYPE in "HDF5"; do
					...
					if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
						...
						#JUST FOR HDF5
						if [[ ${FILE_TYPE} == "HDF5" ]]; then

							#CHECK SEX
							if [[ ${CTRL_SEX_TEST} == "default" || ${CTRL_SEX_TEST} == "yes" ]]; then
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ controls prep - "${ECHO_PATTERN}": predicting sex - "${FILE_TYPE_END}${COL2}
								echo -e ${COL1}${END}${COL2}
								CTRL_SEX_PREDICTION=`Rscript --vanilla ${R}"ctrl_predict_sex.R" ${CTRL_COUNT_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5"`
								...
								if [[ ${CTRL_SEX} == "unk" ]]; then 
									if [[ ${CTRL_SEX_PREDICTION} != "unk" ]]; then
										echo -e ${COL1_WARN}"◼︎ INFO - Sex of control with unknonw sex predicted"
										echo -e "   Project: "${PROJECT_ID}
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Control ID: "${CTRL_ID}
										...
										echo -e ""${COL2}
										mv ${CTRL_COUNT_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5" ${CTRL_COUNT_DIR}${MAIN_ID}"_"${CTRL_ID}"_"${CTRL_SEX_PREDICTION}"_counts.hdf5"
									fi
								fi
								...
								if [[ ${CTRL_SEX} != "unk" ]]; then 
									if [[ ${CTRL_SEX_PREDICTION} != "unk" ]]; then 
										if [[ ${CTRL_SEX} == ${CTRL_SEX_PREDICTION} ]]; then 
											echo -e ${COL1_WARN}"◼︎ INFO - Control sex successfully verified"
											echo -e "   Project: "${PROJECT_ID}
											echo -e "   Main ID: "${MAIN_ID}
											echo -e "   Control ID: "${CTRL_ID}
				...
				#redefine sex for "unk" if just generated
				if [[ ${CTRL_SEX} == "unk" ]]; then 
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_M_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="M"; fi
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_F_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="F"; fi
					if ls ${CTRL_COUNT_HDF_DIR}${MAIN_ID}"_"${CTRL_ID}"_unk_counts.hdf5" 1> /dev/null 2>&1; then CTRL_SEX="unk"; fi
				...
				if [[ ${AUTOSOME_MODE} != "A" && ${CTRL_SEX} == "M" ]]; then
					OUTPUT=${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5"
					if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
						echo -e ${COL1}${BEG}${COL2}
						echo -e ${COL1}"◼︎ $(date)"${COL2}
						echo -e ${COL1}"◼︎ controls prep - "${ECHO_PATTERN}": doubling gonosomal coverage for this male control"${COL2}
						echo -e ${COL1}${END}${COL2}
						Rscript --vanilla ${R}"gonosome_double.R" \
							${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5" \
							${OUTPUT}
						...
						if [[ ${CHECKPOINTS} == "yes" ]]; then 
							if ls "status_ok" 1> /dev/null 2>&1; then
								...
								echo -e "   Main ID: "${MAIN_ID}
								echo -e "   Control ID: "${CTRL_ID}
								echo -e "   Stage: doubling gonosomal counts for male control"
								echo -e "   File not generated: "${OUTPUT}
								echo -e "   R script: gonosome_double.R"
								echo -e ""${COL2}
								touch ./error
								exit
							fi
			...
			cat ${MASTERCONTROLS} | awk -F"\t" -v PROJECT_ID="${PROJECT_ID}" -v CAPTURE_ID="${CAPTURE_ID}" -v GENOME_VERSION="${GENOME_VERSION}" '{if ($1=="yes" && $2==PROJECT_ID && $3==CAPTURE_ID && $4==GENOME_VERSION)print}' | while read -r line || [[ -n "$line" ]]; do
				...
				while IFS= read -r line; do
					...
					file=$(echo "$line" | awk -F'\t' -v CTRL_COUNT_HDF_DIR="${CTRL_COUNT_HDF_DIR}" '{print CTRL_COUNT_HDF_DIR $5 "_" $6 "_" $7 "_counts.hdf5"}')
					if [[ $defined_sex == "unk" && ! -e "$file" ]]; then
						base_filename=${file%_unk_counts.hdf5}
						[[ -e ${base_filename}"_M_counts.hdf5" ]] && NEWSEX="M"
						[[ -e ${base_filename}"_F_counts.hdf5" ]] && NEWSEX="F"
				...
				if [[ ${NReal} != 0 ]]; then
					...
					OUTPUT_DENOIS=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_denoisedCR_PoN-"${PON_SEX_ID}".tsv"
					if ! ls ${OUTPUT_DENOIS} 1> /dev/null 2>&1; then
						...
						if [[ ${AUTOSOME_MODE} != "A" && ${PON_SEX_ID} == "M" ]]; then				
							#prepare PoN
							OUTPUT=${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5"
							if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ controls prep - "${ECHO_PATTERN}": preparing PoN-"${PON_SEX_ID}" - with gonosomal double"${COL2}
								echo -e ${COL1}${END}${COL2}
								mkdir -p ${CTRL_PoN_DIR}
								...
								${GATK} CreateReadCountPanelOfNormals \
					 			...
					 			if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
										...
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Control ID: "${CTRL_ID}
										echo -e "   Stage: panel of normals - with gonosomal double"
										echo -e "   File not generated: "${OUTPUT}
										echo -e ""${COL2}
										touch ./error
										exit
				...
				OUTPUT_ST=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_standardizedCR_PoN-"${PON_SEX_ID}".tsv"
				if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
					...
					if [[ ${NReal} != 0 ]]; then
						...
						${GATK} DenoiseReadCounts \
							...
							--standardized-copy-ratios ${OUTPUT_ST} \
						...
						#prepare secondary denois CN data if required, with doubled gonosomal counts and replace gonosomes in the denois/standard data
						if [[ ${AUTOSOME_MODE} != "A" ]]; then
							
							#join condition for all situations when secondary denois should be generated
							if [[ "${CTRL_SEX}" == "M" && "${PON_SEX_ID}" == "M" ]] || [[ "${CTRL_SEX}" == "M" && "${PON_SEX_ID}" == "F" ]] || [[ "${CTRL_SEX}" == "F" && "${PON_SEX_ID}" == "M" ]] || [[ "${CTRL_SEX}" == "unk" && "${PON_SEX_ID}" == "M" ]]; then 

								OUTPUT_PRIMARY=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_denoisedCR_PoN-"${PON_SEX_ID}"_singlegon.tsv"
								mv ${OUTPUT} ${OUTPUT_PRIMARY}
								OUTPUT_SECONDARY=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_denoisedCR_PoN-"${PON_SEX_ID}"_doublegon.tsv"
								OUTPUT_SECONDARY_ST=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_standardizedCR_PoN-"${PON_SEX_ID}"_doublegon.tsv"

								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ controls prep - "${ECHO_PATTERN}": preparing secondary denoising CN data with doubled gonosomal counts"${COL2}
								echo -e ${COL1}${END}${COL2}
								
								if [[ ${CTRL_SEX} == "M" && ${PON_SEX_ID} == "M" ]]; then
									${GATK} DenoiseReadCounts \
										-I ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5" \
										--count-panel-of-normals ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5" \
										--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
										--denoised-copy-ratios ${OUTPUT_SECONDARY}
								fi
								if [[ ${CTRL_SEX} == "M" && ${PON_SEX_ID} == "F" ]]; then
									${GATK} DenoiseReadCounts \
										-I ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5" \
										--count-panel-of-normals ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".hdf5" \
										--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
								...
								if [[ ${CTRL_SEX} == "F" && ${PON_SEX_ID} == "M" ]]; then
									${GATK} DenoiseReadCounts \
										-I ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5" \
										--count-panel-of-normals ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5" \
										--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
								...
								if [[ ${CTRL_SEX} == "unk" && ${PON_SEX_ID} == "M" ]]; then
									${GATK} DenoiseReadCounts \
										-I ${CTRL_COUNT_HDF_DIR}${RESULTS_DATA_PATTERN}"_counts.hdf5" \
										--count-panel-of-normals ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5" \
										--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
								...
								rm ${OUTPUT_SECONDARY_ST}
								if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ! ls ${OUTPUT_SECONDARY} 1> /dev/null 2>&1; then
										echo -e ${COL1_INFO}"◼︎ CHECKPOINT ERROR - CONTROL PREP: file not generated"
										echo -e "   Project: "${PROJECT_ID}
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Control ID: "${CTRL_ID}
										echo -e "   Stage: CN standardization and denoising - creating secondary denois file with gonosomal double count"
										echo -e "   File not generated: "${OUTPUT_SECONDARY}
										echo -e ""${COL2}
										touch ./error
										exit
									fi
								...
								#if secondary denois was generated, replace gonosomal part in the primary output
								OUTPUT_MERGED=${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_denoisedCR_PoN-"${PON_SEX_ID}"_merged.tsv"
								echo -e ${COL1}${BEG}${COL2}
								echo -e ${COL1}"◼︎ $(date)"${COL2}
								echo -e ${COL1}"◼︎ controls prep - "${ECHO_PATTERN}": replacing gonosomal counts in the original denoised CN data"${COL2}
								echo -e ${COL1}${END}${COL2}
								#export header
								cat ${OUTPUT_PRIMARY} | grep "^@" > ${CTRL_DENOIS_DIR}"header"
								#combine data
								Rscript --vanilla ${R}"gonosome_double_joint_denois.R" \
									${CONTIGS_SIZES} \
									${OUTPUT_PRIMARY} \
									${OUTPUT_SECONDARY} \
									${OUTPUT_MERGED}
								...
								if [[ ${CHECKPOINTS} == "yes" ]]; then 
									if ls "status_ok" 1> /dev/null 2>&1; then
										...
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Control ID: "${CTRL_ID}
										echo -e "   Stage: joining denoised data with gonosomal double"
										echo -e "   File not generated: "${OUTPUT_MERGED}
										echo -e "   R script: gonosome_double_joint_denois.R"
										echo -e ""${COL2}
										touch ./error
										exit
									fi
									if ! ls ${OUTPUT_MERGED} 1> /dev/null 2>&1; then
										echo -e ${COL1_INFO}"◼︎ CHECKPOINT ERROR - CONTROL PREP: file not generated"
										echo -e "   Project: "${PROJECT_ID}
										echo -e "   Main ID: "${MAIN_ID}
										echo -e "   Control ID: "${CTRL_ID}
								...
								#add original header into the merged file
								cat ${CTRL_DENOIS_DIR}"header" ${OUTPUT_MERGED} > ${OUTPUT}
								rm ${OUTPUT_PRIMARY} ${OUTPUT_SECONDARY} ${OUTPUT_MERGED} ${CTRL_DENOIS_DIR}"header"
							fi
						fi
					fi
					...
					#delete PON files to save some space
					rm -f ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".hdf5"
					rm -f ${CTRL_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5"
				fi
				...
				if [[ ${QUICK_MODE} != "Q" ]]; then
					...
					if ! ls ${OUTPUT} 1> /dev/null 2>&1; then 
						...
						Rscript --vanilla ${R}"segmentation_and_qc.R" \
							...
							${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_standardizedCR_PoN-"${PON_SEX_ID}".tsv" \
							${CTRL_DENOIS_DIR}${RESULTS_DATA_PATTERN}"_denoisedCR_PoN-"${PON_SEX_ID}".tsv" \
							${MAIN_ID} \
							${CTRL_ID} \
							"control" \
	...
	#preparation for CNVkit
	###====================================================================================
	#CHECK - CTRL YES 
	if [[ ${CNVKIT_MODE} == "Y" ]]; then
		
		echo -e ${COL1}${BEG}${COL2}
		echo -e ${COL1}"◼︎ $(date)"${COL2}
		echo -e ${COL1}"◼︎ PREPROCESSING - entering CNVkit preparation stage"${COL2}
		echo -e ${COL1}${END}${COL2}

		HERE_PATH=`pwd`
		source ${HERE_PATH}"/sh/run_01_prep_cnvkit.sh"
