...
cat ${MASTERPROJECTS} | awk -F"\t" '$1=="yes"' | while read -r line || [[ -n "$line" ]]; do
	...
	cat ${MASTERSAMPLES} | awk -F"\t" -v PROJECT_ID="${PROJECT_ID}" -v CAPTURE_ID="${CAPTURE_ID}" -v GENOME_VERSION="${GENOME_VERSION}" '{if ($1=="yes" && $2==PROJECT_ID && $3==CAPTURE_ID && $4==GENOME_VERSION)print}' | while read -r line || [[ -n "$line" ]]; do
		...
		for CASE_ID in ${SAMPLE_1_ID} ${SAMPLE_2_ID} ${SAMPLE_3_ID}; do
			...
			if ls ${BAMFILE} 1> /dev/null 2>&1; then
				...
				if [[ ${AUTOSOME_MODE} != "A" && ${CASE_SEX} == "M" ]]; then
					OUTPUT=${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5"
					#CHECK - READ COUNTS
					if ! ls ${OUTPUT} 1> /dev/null 2>&1; then

						#CHECK - AS CTRL EXISTS
						if ls ${CTRL_COUNT_COUNTS_DIR}"hdf5/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_counts_doublegon.hdf5" 1> /dev/null 2>&1; then
							echo -e ${COL1}${BEG}${COL2}
							echo -e ${COL1}"◼︎ $(date)"${COL2}
							echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": collecting read count - hdf5 double gonosomes"${COL2} 
							echo -e ${COL1}"◼︎ collected as control, copying data..."${COL2} 							
							echo -e ${COL1}${END}${COL2}
							cp ${CTRL_COUNT_COUNTS_DIR}"hdf5/"${MAIN_ID}"_"${CASE_ID}"_"${CASE_SEX}"_counts_doublegon.hdf5" ${OUTPUT}
						else
							echo -e ${COL1}${BEG}${COL2}
							echo -e ${COL1}"◼︎ $(date)"${COL2}
							echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": doubling gonosomal coverage for this male sample"${COL2}
							echo -e ${COL1}${END}${COL2}
							Rscript --vanilla ${R}"gonosome_double.R" \
								${RESULTS_DATA_PATTERN}"_counts.hdf5" \
								${OUTPUT}
							...
							if [[ ${CHECKPOINTS} == "yes" ]]; then 
								if ls "status_ok" 1> /dev/null 2>&1; then
									...
									echo -e "   Main ID: "${MAIN_ID}
									echo -e "   Case: "${CASE_ID}" ("${CASE_TYPE}")"
									echo -e "   Stage: doubling gonosomal counts for male sample"
									echo -e "   File not generated: "${OUTPUT}
				...
				for PON_SEX in ${PON_SEX1} ${PON_SEX2} ${PON_SEX3} ${PON_SEX4} ${PON_SEX5}; do
					...
					if [[ ${PON_SEX} != "NA" ]]; then
						...
						if [[ ${NReal} != 0 ]]; then
							...
							OUTPUT_DENOIS=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}".tsv"
							#CHECK - DENOIS
							if ! ls ${OUTPUT_DENOIS} 1> /dev/null 2>&1; then

								if [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
									OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline.hdf5"
								else
								...
								if [[ ${AUTOSOME_MODE} != "A" ]]; then
									if [[ ${PON_SEX_ID} == "M" ]] || [[ ${GERMLINE_MODE} == "G" && ${CASE_SEX} == "M" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
									
										if [[ ${GERMLINE_MODE} == "G" && ${CASE_SEX} == "M" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
											OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline_doublegon.hdf5"
										else
											OUTPUT=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5"
										fi
										...
										if ! ls ${OUTPUT} 1> /dev/null 2>&1; then
											echo -e ${COL1}${BEG}${COL2}
											echo -e ${COL1}"◼︎ $(date)"${COL2}
											echo -e ${COL1}"◼︎ "${MAIN_ID}": preparing PoN-"${PON_SEX_ID}" - with gonosomal double"${COL2}
											echo -e ${COL1}${END}${COL2}
						...
						OUTPUT_ST=${RESULTS_DATA_PATTERN}"_standardizedCR_C-"${N}"_PoN-"${PON_SEX_ID}".tsv"

						if [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_1_ID} ]]; then
							PANEL=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline.hdf5"
						else
							PANEL=${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".hdf5"
						fi
						...
						if ! ls ${OUTPUT} 1> /dev/null 2>&1; then 
							...
							if [[ ${NReal} != 0 ]]; then 
								...
								#CHECK - AUTOSOMAL MODE 
								if [[ ${AUTOSOME_MODE} != "A" ]]; then

									# #CHECK - CONDITION 
									# # join condition for all situations when secondary denois should be generated
									# if [[ "${CASE_SEX}" == "M" && "${PON_SEX_ID}" == "M" ]] || [[ "${CASE_SEX}" == "M" && "${PON_SEX_ID}" == "F" ]] || [[ "${CASE_SEX}" == "F" && "${PON_SEX_ID}" == "M" ]] || [[ "${CASE_SEX}" == "unk" && "${PON_SEX_ID}" == "M" ]] || [[ "${GERMLINE_MODE}" == "G" && "${CASE_SEX}" == "M" && ]]; then

										OUTPUT_PRIMARY=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}"_singlegon.tsv"
										OUTPUT_SECONDARY=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}"_doublegon.tsv"
										OUTPUT_SECONDARY_ST=${RESULTS_DATA_PATTERN}"_standardizedCR_C-"${N}"_PoN-"${PON_SEX_ID}"_doublegon.tsv"

										# echo -e ${COL1}${BEG}${COL2}
										# echo -e ${COL1}"◼︎ $(date)"${COL2}
										# echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": secondary denoising read count using PoN-"${PON_SEX_ID}" with doubled gonosomal counts"${COL2} 
										# echo -e ${COL1}${END}${COL2}	


										if [[ ${GERMLINE_MODE} != "G" ]] || [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_2_ID} ]]; then	
											if [[ ${PON_SEX_ID} == "M" ]] || [[ ${CASE_SEX} == "M" ]]; then
												...
												if [[ ${CASE_SEX} == "M" && ${PON_SEX_ID} == "M" ]]; then
													${GATK} DenoiseReadCounts \
														-I ${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5" \
														--count-panel-of-normals ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}"_doublegon.hdf5" \
												...
												if [[ ${CASE_SEX} == "M" && ${PON_SEX_ID} == "mixed" ]]; then
													${GATK} DenoiseReadCounts \
														-I ${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5" \
														--count-panel-of-normals ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"${PON_SEX_ID}".hdf5" \
														--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
										...
										if [[ ${GERMLINE_MODE} == "G" && ${CASE_ID} == ${SAMPLE_1_ID} && ${CASE_SEX} == "M" ]]; then
											mv ${OUTPUT} ${OUTPUT_PRIMARY} 
											echo -e ${COL1}${BEG}${COL2}
											echo -e ${COL1}"◼︎ $(date)"${COL2}
											echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": secondary denoising read count using PoN-"${PON_SEX_ID}" with doubled gonosomal counts"${COL2} 
											...
											${GATK} DenoiseReadCounts \
												-I ${RESULTS_DATA_PATTERN}"_counts_doublegon.hdf5" \
												--count-panel-of-normals ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-germline_doublegon.hdf5" \
												--standardized-copy-ratios ${OUTPUT_SECONDARY_ST} \
												--denoised-copy-ratios ${OUTPUT_SECONDARY}
										...
										if ls ${OUTPUT_SECONDARY} 1> /dev/null 2>&1; then
											...
											OUTPUT_MERGED=${RESULTS_DATA_PATTERN}"_denoisedCR_C-"${N}"_PoN-"${PON_SEX_ID}"_merged.tsv"
											echo -e ${COL1}${BEG}${COL2}
											echo -e ${COL1}"◼︎ $(date)"${COL2}
											echo -e ${COL1}"◼︎ "${MAIN_ID}" - "${CASE_TYPE}" - "${CASE_ID}": replacing gonosomal counts in the original denoising CN data"${COL2} 
											echo -e ${COL1}${END}${COL2}
											#export header
											cat ${OUTPUT_PRIMARY} | grep "^@" > ${RESULTS_DATA_DIR}"header"
											#combine data
											...
											cat ${RESULTS_DATA_DIR}"header" ${OUTPUT_MERGED} > ${OUTPUT}
											rm ${OUTPUT_PRIMARY} ${OUTPUT_SECONDARY} ${OUTPUT_MERGED} ${RESULTS_DATA_DIR}"header"
										fi
							...
							#delete PON hdf5 files to save some space
							rm -f ${SAMPLE_PoN_DIR}${MAIN_ID}"_PoN-"*".hdf5"
		...
		for PON_SEX in ${PON_SEX1} ${PON_SEX2} ${PON_SEX3} ${PON_SEX4} ${PON_SEX5}; do
			...
			if [[ ${PON_SEX} != "NA" ]]; then
				...
				###====================================================================================
				#CHECK - CTRL YES 
				if [[ ${ASCAT_MODE} == "Y" ]]; then
					
					echo -e ${COL1}${BEG}${COL2}
					echo -e ${COL1}"◼︎ $(date)"${COL2}
					echo -e ${COL1}"◼︎ PREPROCESSING - entering ASCAT analysis"${COL2}
					echo -e ${COL1}${END}${COL2}

					HERE_PATH=`pwd`
					source ${HERE_PATH}"/sh/run_02_analysis_ascat.sh"
	...
	if [[ ${CNVKIT_MODE} == "Y" ]]; then
		...
		echo -e ${COL1}${BEG}${COL2}
		echo -e ${COL1}"◼︎ $(date)"${COL2}
		echo -e ${COL1}"◼︎ PREPROCESSING - entering CNVkit analysis stage"${COL2}
		echo -e ${COL1}${END}${COL2}

		HERE_PATH=`pwd`
		source ${HERE_PATH}"/sh/run_02_analysis_cnvkit.sh"
