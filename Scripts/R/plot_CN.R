...
for (i in list(paste0(SAMPLE1_TYPE, "_", SAMPLE1_ID, "_", SAMPLE1_SEX), paste0(SAMPLE2_TYPE, "_", SAMPLE2_ID, "_", SAMPLE2_SEX), paste0(SAMPLE3_TYPE, "_", SAMPLE3_ID, "_", SAMPLE3_SEX))) {
  ...
  if(file.exists(paste0(INDIR, i, "_SEGMENTS_C-", CTRL_N, "_", PON, "_", GETSEGMENTCONDITION, MODEL_FILE_PATTERN))) {
    ...
    write_tsv(IGV_MODEL_BED_CN, paste0(OUTDIR_IGV, GROUP, "_", i, "_abnormal_CN_segments.bed"), col_names = F)
    
    IGV_MODEL_BED_LOH = IGV_MODEL %>% 
      filter(TYPE != "NORMAL") %>%
      filter(TYPE == "HOM" | TYPE == "OTHER") %>%
      select(CONTIG, START, END, TYPE, x1, x2, START2, END2, COLOR)
    if (nrow(IGV_MODEL_BED_LOH) > 0) {write_tsv(IGV_MODEL_BED_LOH, paste0(OUTDIR_IGV, GROUP, "_", i, "_abnormal_LOH_segments.bed"), col_names = F)}
...
CUSTOM_CYTOBAND  = toGRanges(DTB_CHROMBAND)
...
RES = 300
if (("TRUE" %in% grepl("chr", CONTIGS_in_SAMPLE))==TRUE) {
  MIN = vlookup("chr21", dict = DTB_CHRSIZE, result_column = 2, lookup_column = 1)
} else {
  MIN = vlookup("21", dict = DTB_CHRSIZE, result_column = 2, lookup_column = 1)
}
...
#skip details when quick mode
if(DETAIL_PLOT_SKIP != "Q") {
