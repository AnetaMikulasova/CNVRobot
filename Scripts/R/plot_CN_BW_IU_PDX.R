...
#CHM13 - liftover from hg38
#	MYC	chr8	126121230	131114107
#	IGH	chr14	99785131	101158993
#	IGK	chr2	88212514	91002950
#	IGL	chr22	22059213	24104965
#	CCND1	chr11	69042767	69802152
#	MAF	chr16	83522217	86026809
#	FGFR3NSD2	chr4	1696148	2097066
...
DTB_IG_ANNOT_REG = read.delim("/media/aneta/Data/databases/T2T/bed_files_mine/gencode_genes_CHM13_V31_liftover_IG_VDJC.bed", header = FALSE, stringsAsFactors = F)
...
DTB_BLUEPRINT = read.delim("/media/aneta/Data/databases/T2T/bed_files_mine/regions_results_NEW_NEW_CHM13.bed", header = FALSE, stringsAsFactors = F) %>%
  select(V1, V2, V3, V4) %>%
...
MYC = c("MYC-region", "chr8", 126121230, 131114107)
CCND1 = c("CCND1-region", "chr11", 69042767, 69802152)
MAF = c("MAF-region", "chr16", 83522217, 86026809)
FGFR3NSD2 = c("FGFR3NSD2-region", "chr4", 1696148, 2097066)
# IGH = c("IGH-region", "chr14", 99785131, 101158993) #general IGH
IGH = c("IGH-region", "chr14", 99604841, 101158993) #just for the TI in the sample to see whole gain at IGH region - 99654841

IGK = c("IGK-region", "chr2", 88212514, 91002950)
IGL = c("IGL-region", "chr22", 22059213, 24104965)


REGIONSofINTEREST = rbind(REGIONSofINTEREST, MYC, CCND1, MAF, FGFR3NSD2, IGH, IGK, IGL)
# REGIONSofINTEREST = rbind(REGIONSofINTEREST, IGH)
